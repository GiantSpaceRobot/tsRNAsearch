...
function bam_to_plots () {  ### Steps for plotting regions with high variation in coverage
	...
	if [ $3 = "tiRNA" ]; then
		...
		Rscript scripts/Remove-introns.R $1/accepted_hits.genomecov additional-files/tRNA-introns-for-removal_hg19.tsv $1/accepted_hits_intron-removed.genomecov 
		### Flip the read coverage of tRNAs that are in the minus orientation
		Rscript scripts/Coverage-flipper.R $1/accepted_hits_intron-removed.genomecov DBs/hg19-wholetRNA-CCA.gtf $1/accepted_hits_flipped.genomecov	
		### Collapse tRNAs from the same tRNA species
		python scripts/Bedgraph_collapse-tRNAs.py $1/accepted_hits_flipped.genomecov $1/accepted_hits_collapsed.genomecov
...
if [[ $pairedEnd = "True" ]]; then
	...
	if [[ $oldAligner = "yes" ]]; then
		...
		echo $trimmedFile1
		hisat2 -p $CPUs -x DBs/hisat2_index/hg19-wholetRNA-CCA -1 $outDir/trim_galore_output/$trimmedFile1 -2 $outDir/trim_galore_output/$trimmedFile2 -S $outDir/tRNA-alignment/aligned_tRNAdb.sam
	...
	if [[ $oldAligner = "yes" ]]; then
		...
		if [ ! -f $outDir/tRNA-alignment/align_summary.txt ]; then
			string_padder "Running tRNA alignment step..."
			hisat2 -p $CPUs -x DBs/hisat2_index/hg19-wholetRNA-CCA -U $outDir/trim_galore_output/$trimmedFile -S $outDir/tRNA-alignment/aligned_tRNAdb.sam --summary-file $outDir/tRNA-alignment/align_summary.txt --un $outDir/tRNA-alignment/unmapped.fastq
		...
		if [ ! -f $outDir/snomiRNA-alignment/$trimmedFile ]; then # If this file was not generated, try and align the unmapped reads from the tRNA alignment
			string_padder "Running sno/miRNA alignment step..."
			hisat2 -p $CPUs -x DBs/hisat2_index/hg19-snomiRNA $outDir/tRNA-alignment/$trimmedFile -S $outDir/snomiRNA-alignment/aligned.sam --summary-file $outDir/snomiRNA-alignment/align_summary.txt --un $outDir/snomiRNA-alignment/unmapped.fastq
			#hisat2 -p $CPUs -x DBs/hisat2_index/hg19-snomiRNA -U $outDir/tRNA-alignment/$trimmedFile -S $outDir/snomiRNA-alignment/aligned.sam --summary-file $outDir/snomiRNA-alignment/align_summary.txt --un $outDir/snomiRNA-alignment/unmapped.fastq
		fi
		if [ ! -f $outDir/snomiRNA-alignment/align_summary.txt ]; then # If this file was still not generated, use the unmapped FASTQ from the first alignment output
			echo "
				snomiRNA alignment output not found. Reads likely did not map to sno/miRNA reference. 
		...
		if [ ! -f $outDir/mRNA-ncRNA-alignment/align_summary.txt ]; then # If this file was not generated, try and align the unmapped reads from the tRNA alignment	
			...
			if [ -f $outDir/mRNA-ncRNA-alignment/aligned.sam ]; then  #If hisat2 successfully mapped reads, convert the unmapped to FASTQ
	            ...
	            samtools sort $outDir/mRNA-ncRNA-alignment/accepted_hits_unsorted.bam $outDir/mRNA-ncRNA-alignment/accepted_hits
				samtools index $outDir/mRNA-ncRNA-alignment/accepted_hits.bam
	            cp $outDir/mRNA-ncRNA-alignment/unmapped.fastq $outDir/mRNA-ncRNA-alignment/$trimmedFile
