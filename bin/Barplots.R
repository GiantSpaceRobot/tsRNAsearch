...
if (substring(path.to.files, nchar(path.to.files)) == "/") {
  ...
  path.to.files <- paste0(path.to.files, "/")
}
...
groups <- factor(x=c(rep(Condition1, ReplicateNumber1), rep(Condition2, ReplicateNumber2)), levels=c(Condition1, Condition2))
  ...
  write.table(x = flipped.percent.df, file = paste0(path.to.files, "Percent-Feature-Per-Sample.tsv"), quote = FALSE, sep = "\t", col.names=NA)
  ...
  write.table(x = my.pval.df, file = paste0(path.to.files, "Percent-Feature-Per-Sample_T-tests.tsv"), quote = FALSE, sep = "\t", col.names=NA)
  ...
  ggsave(width = 7, filename = paste0(path.to.files, "Barplot_All-Samples.pdf"), plot = myplot) # Save plot using ggplot2 ggsave
